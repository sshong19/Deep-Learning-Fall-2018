...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    if self.activation is 'relu':
        ...
        compute_real_mask = np.logical_and(-limit < z1, z1 < limit)
        return_same_mask = z1 >= limit
        computed_real_part = np.log(1 + np.exp(z1*compute_real_mask)*compute_real_mask)
        returned_same_part = z1*return_same_mask
        hidden =  computed_real_part + returned_same_part
        
        
    elif self.activation is 'sigmoid':
        hidden = np.zeros([N,Hs])
        hidden[z1>-limit] = 1/(1+np.exp(-z1[z1>-limit]))
        hidden[z1<=-limit] = 0
    ...
    scores_mod = scores - np.max(scores, 1).reshape(-1,1)
    h = np.exp(scores_mod)
    h = h/np.sum(h, 1).reshape(-1,1)
    logh = scores_mod - np.log(np.sum(np.exp(scores_mod) , 1)).reshape(-1,1)
    loss = -np.sum(logh[range(N),y])/N
...
# output layer
    ...
    yhot[range(N),y] = 1
    dscore = h - yhot
    dW2 = np.dot(hidden.T, dscore)/N + reg*W2
    db2 = np.sum(dscore, 0) /N
    ...
    if self.activation is 'relu':
        ...
        dz1 = np.zeros([N,Hs])
        dz1[z1>-limit] = 1/(1+np.exp(-z1[z1>-limit]))*dhidden[z1>-limit]
        dz1[z1<=-limit] = 0
